...
def tournament():
    ...
    achievements = db.execute(f"SELECT a.*, bc.name AS break_category_name, , s.last_name, s.first_name FROM achievements a LEFT JOIN break_categories bc ON a.break_category = bc.id LEFT JOIN speaker_categories sc ON a.speaker_category = sc.id INNER JOIN speakers s on a.speaker_id = s.id WHERE a.tournament_id = {id} AND (a.name = ? OR a.type = ? OR a.type = ?)",
