...
def speaker():
    ...
    achievements = db.execute(f"SELECT a.*, bc.name,  FROM achievements a LEFT JOIN break_categories bc ON a.break_category = bc.id LEFT JOIN speaker_categories sc ON a.speaker_category = sc.id WHERE speaker_id = {id} ORDER BY id DESC")
